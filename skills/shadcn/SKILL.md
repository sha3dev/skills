...
metadata:
  ...
  modified: Adapted for portable project-context discovery and automatic invocation.
---
...
Before changing an existing shadcn project, run `shadcn info --json` with the
project's package runner from the application workspace. The result contains
the project config and installed components. Do not initialize shadcn merely
because a project uses React; initialization must be requested or already
represented by a `components.json` file. Use `shadcn docs <component>` with the
same runner to get documentation and example URLs for any component.
...
1. **Get project context** — run `npx shadcn@latest info --json`, substituting the project's package runner. If this is an explicitly requested initialization and no `components.json` exists yet, inspect the target framework and package manager before running `init`.
